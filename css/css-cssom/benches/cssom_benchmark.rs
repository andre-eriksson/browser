...
use css_cssom::{CSSRule, CSSStyleRule, CSSStyleSheet};
...
fn bench_get_style_rules(c: &mut Criterion) {
    ...
    let stylesheet = CSSStyleSheet::from_css(COMPLEX_CSS);
...
fn bench_css_rules_access(c: &mut Criterion) {
    let stylesheet = CSSStyleSheet::from_css(MULTIPLE_RULES_CSS);
...
fn bench_insert_delete_rule(c: &mut Criterion) {
    ...
    group.bench_function("insert_rule", |b| {
        b.iter_batched(
            || CSSStyleSheet::from_css(SIMPLE_CSS),
            |mut stylesheet| {
    ...
    group.bench_function("delete_rule", |b| {
        b.iter_batched(
            || CSSStyleSheet::from_css(MULTIPLE_RULES_CSS),
            |mut stylesheet| {
...
fn bench_property_access(c: &mut Criterion) {
    let stylesheet = CSSStyleSheet::from_css(DECLARATION_HEAVY_CSS);
