...
pub fn is_simple_headers(headers: &HeaderMap) -> bool {
    for (name, value) in headers {
        if !matches!(*name, ACCEPT | ACCEPT_LANGUAGE | CONTENT_LANGUAGE | CONTENT_TYPE | RANGE) {
            return false;
        }
...
pub fn is_simple_header(header_name: &HeaderName, header_value: &HeaderValue) -> bool {
    ...
    match *header_name {
        ...
        ACCEPT_LANGUAGE | CONTENT_LANGUAGE => {
        ...
        CONTENT_TYPE => {
            ...
            if !is_simple_content_type(header_value) {
                return false;
            }
        }
        RANGE => {
            if !is_simple_range(header_value) {
                return false;
            }
        }
        _ => {
