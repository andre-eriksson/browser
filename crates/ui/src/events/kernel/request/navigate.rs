use iced::Task;
use kernel::{Commandable, KernelCommand, KernelResponse, errors::KernelError};
use regex::Regex;
use url::Url;
...
pub(crate) fn navigate_to_url(application: &mut Application, new_url: String) -> Task<Event> {
    ...
    Task::perform(
        async move {
            ...
            lock.execute(KernelCommand::Navigate {
                tab_id: active_tab,
                url,
            })
            .await
        ...
        |result| match result {
            ...
            Err(err) => match err {
                KernelError::NavigationError(nav_err) => Event::KernelResponse(KernelResponse::NavigateError(nav_err)),
                _ => Event::KernelResponse(KernelResponse::Error(err)),
            },
        },
    )
}
...
pub(crate) fn navigate_back(application: &mut Application) -> Task<Event> {
    ...
    Task::perform(
        async move {
            let mut lock = browser.lock().await;
            lock.execute(KernelCommand::NavigateBack { tab_id: active_tab })
                .await
        },
        |result| match result {
            Ok(event) => Event::KernelResponse(event),
...
pub(crate) fn navigate_forward(application: &mut Application) -> Task<Event> {
    ...
    Task::perform(
        async move {
            let mut lock = browser.lock().await;
            lock.execute(KernelCommand::NavigateForward { tab_id: active_tab })
                .await
        },
        |result| match result {
            Ok(event) => Event::KernelResponse(event),
