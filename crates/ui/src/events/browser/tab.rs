use iced::Task;
use kernel::{Commandable, KernelCommand, KernelResponse, TabId};
...
pub(crate) fn create_new_tab(application: &mut Application) -> Task<Event> {
    ...
    Task::perform(
        async move {
            ...
            lock.execute(KernelCommand::AddTab).await
        },
        |result| match result {
            Ok(event) => Event::KernelResponse(event),
            Err(err) => Event::KernelResponse(KernelResponse::Error(err)),
        },
    )
}
...
pub(crate) fn close_tab(application: &mut Application, tab_id: TabId) -> Task<Event> {
    ...
    Task::perform(
        async move {
            let mut lock = browser.lock().await;
            lock.execute(KernelCommand::CloseTab { tab_id }).await
        },
        |result| match result {
            Ok(event) => Event::KernelResponse(event),
            Err(err) => Event::KernelResponse(KernelResponse::Error(err)),
...
pub(crate) fn change_active_tab(application: &mut Application, tab_id: TabId) -> Task<Event> {
    ...
    Task::perform(
        async move {
            let mut lock = browser.lock().await;
            lock.execute(KernelCommand::ChangeActiveTab { tab_id })
                .await
        },
        |result| match result {
            Ok(event) => Event::KernelResponse(event),
