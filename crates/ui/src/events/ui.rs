...
use crate::{
    ...
    events::{
        ...
        ui::{
            ...
            window::{
                close_window, create_window, on_content_scrolled, on_url_change, on_window_resized,
            },
        },
    },
};
    ...
    fn handle(&mut self, event: UiEvent) -> Task<Event> {
        match event {
            ...
            UiEvent::WindowResized(window_id, width, height) => {
                on_window_resized(self, window_id, width, height)
            }
            ...
            UiEvent::ImageLoaded(tab_id, ref url, ref vary_key) => {
                on_image_loaded(self, tab_id, url, vary_key)
            }
