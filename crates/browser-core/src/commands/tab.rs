...
pub(crate) fn add_tab(tab_manager: &mut TabManager) -> EngineResponse {
    ...
    EngineResponse::TabAdded(new_tab_id)
}

/// Closes the tab with the specified `tab_id`. If the closed tab is the active tab,
pub(crate) fn close_tab(tab_manager: &mut TabManager, tab_id: TabId) -> Result<EngineResponse, KernelError> {
    ...
    if tab_manager.active_tab_id() == tab_id {
        ...
        return Ok(EngineResponse::TabClosed(tab_id, Some(new_tab_id)));
    }

    Ok(EngineResponse::TabClosed(tab_id, None))
}

/// Changes the active tab to the tab with the specified `tab_id`.
pub(crate) fn change_active_tab(tab_manager: &mut TabManager, tab_id: TabId) -> Result<EngineResponse, KernelError> {
    trace!("Changing active tab to ID {:?}", tab_id);

    tab_manager.change_active_tab(tab_id)?;

    Ok(EngineResponse::ActiveTabChanged(tab_id))
}
