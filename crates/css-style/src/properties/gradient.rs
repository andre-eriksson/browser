...
impl Gradient {
    ...
    pub fn parse_function(func: &Function) -> Result<Self, String> {
        if func.name.eq_ignore_ascii_case("linear-gradient") {
            Ok(Self::Linear(LinearGradientSyntax::try_parse(func.value.as_slice())?))
        } else if func.name.eq_ignore_ascii_case("repeating-linear-gradient") {
            Ok(Self::RepeatingLinear(LinearGradientSyntax::try_parse(func.value.as_slice())?))
        } else if func.name.eq_ignore_ascii_case("radial-gradient") {
            Ok(Self::Radial(RadialGradientSyntax::try_parse(func.value.as_slice())?))
        } else if func.name.eq_ignore_ascii_case("repeating-radial-gradient") {
            Ok(Self::RepeatingRadial(RadialGradientSyntax::try_parse(func.value.as_slice())?))
        } else if func.name.eq_ignore_ascii_case("conic-gradient") {
            Ok(Self::Conic(ConicGradientSyntax::try_parse(func.value.as_slice())?))
        } else if func.name.eq_ignore_ascii_case("repeating-conic-gradient") {
            Ok(Self::RepeatingConic(ConicGradientSyntax::try_parse(func.value.as_slice())?))
        } else {
...
/// Helper utilities for parsing gradient-related CSS values.
///
/// These are associated functions on `Gradient` so that all gradient parsing
/// helpers live under a single namespace rather than as standalone free
/// functions.
impl Gradient {
...
mod tests {
    ...
    fn side_or_corner_left() {
        ...
        let stripped = Gradient::strip_whitespace(&cvs);
        let to_pos = stripped
            .iter()
        ...
        let sc = SideOrCorner::try_parse(after_to).unwrap();
    ...
    fn side_or_corner_empty_fails() {
        ...
        assert!(SideOrCorner::try_parse(empty).is_err());
    }
    ...
    fn gradient_linear() {
        ...
        let grad = Gradient::parse_function(func).unwrap();
    ...
    fn gradient_unknown_function_fails() {
        ...
        let func = extract_function(&cvs);
        assert!(Gradient::parse_function(func).is_err());
    }
    ...
    #[test]
    fn gradient_parse_from_stream() {
        let cvs = parse_value("background-image: linear-gradient(red, blue)");
        let grad = Gradient::try_parse(&cvs).unwrap();
        assert!(matches!(grad, Gradient::Linear(_)));
    }
    ...
    fn split_on_commas_basic() {
        ...
        let segments = Gradient::split_on_commas(&cvs);
    ...
    fn try_parse_length_percentage_dimension() {
        ...
        let lp = Gradient::try_parse_length_percentage(meaningful[0]).unwrap();
