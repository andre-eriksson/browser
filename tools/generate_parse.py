...
def generate_rust_match(enum_name, tags):
    ...
    lines.append("    pub fn from_str_insensitive(s: &str) -> Option<Self> {{")
    ...
    for length in sorted(by_len.keys()):
        ...
        lines.append("                match [")
        byte_accessors = ", ".join(
            [f"bytes[{i}].to_ascii_lowercase()" for i in range(length)]
        )
        ...
        for tag in by_len[length]:
            ...
            lines.append(
                f"                    [{byte_pattern}] => Some({enum_name}::{tag}),"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate case-insensitive Rust parser"
    )
